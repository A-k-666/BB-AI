...
class VisionFrameFinder:
    ...
    def find_best_workflow_frame(self, video_path: str, sample_count: int = 20) -> dict:
        ...
        Strategy (UNIVERSAL):
        1. Focus on LAST 20% of video (final workflow typically shown here)
        2. Sample frames densely in that region
        3. Ask Vision to score each frame
        4. Return highest scoring frame
        """
        
        print("Step 2/4: Finding best workflow frame (focused on last 20%)...")
        ...
        # UNIVERSAL: Sample FIRST 20% + LAST 20% (final workflow can be in either)
        first_20_end = duration * 0.20
        last_20_start = duration * 0.80
        
        # Skip intro/outro
        first_region_start = 30
        first_region_end = min(first_20_end, duration * 0.25)
        last_region_start = max(last_20_start, duration * 0.75)
        last_region_end = duration - 15
        
        # Sample half from first 20%, half from last 20%
        half_count = sample_count // 2
        
        # First 20% samples
        first_samples = []
        if first_region_end > first_region_start:
            first_samples = [int(first_region_start * fps + i * (first_region_end - first_region_start) * fps / half_count) 
                           for i in range(half_count)]
        
        # Last 20% samples
        last_samples = []
        if last_region_end > last_region_start:
            last_samples = [int(last_region_start * fps + i * (last_region_end - last_region_start) * fps / half_count) 
                          for i in range(half_count)]
        
        sample_indices = first_samples + last_samples
        
        print(f"   Scanning FIRST 20% ({first_region_start:.1f}s-{first_region_end:.1f}s) + LAST 20% ({last_region_start:.1f}s-{last_region_end:.1f}s)")
